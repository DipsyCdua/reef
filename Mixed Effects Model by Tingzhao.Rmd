...
#1. Mixed Effects Model for mean_dhw
train_data$log_dhw <- log1p(train_data$mean_dhw)

log_model_dhw <- lmer(log_dhw ~ soi_anomaly + mean_sst + mean_ssta + 
                      (1 | shelf) + (1 | year) + (1 | month), data = train_data)
...
#2. Mixed Effects Model for mean_sst
sst_model <- lmer(mean_sst ~ soi_anomaly + mean_ssta + 
                  (1 | shelf) + (1 | year) + (1 | month), data = train_data)

reef_full$pred_sst <- predict(sst_model, newdata = reef_full, allow.new.levels = TRUE)

#3. Mixed Effects Model for mean_ssta
ssta_model <- lmer(mean_ssta ~ soi_anomaly + mean_sst + 
                   (1 | shelf) + (1 | year) + (1 | month), data = train_data)
